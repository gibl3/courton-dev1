...
@section('title', isset($user) ? 'Edit Player - Courton' : 'Add New Player - Courton')
...
<div class="flex flex-col gap-8">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold">{{ isset($user) ? 'Edit Player' : 'Add New Player' }}</h1>
            <p class="text-sm text-neutral-600">{{ isset($user) ? 'Update player information' : 'Create a new player account in the system' }}</p>
        </div>
    ...
    <!-- Form Section -->
    <form action="{{ isset($user) ? route('admin.users.update', $user) : route('admin.users.store') }}" method="POST" class="bg-white rounded-xl border border-neutral-200 p-6">
        @csrf
        @if(isset($user))
        @method('PUT')
        @endif
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <ul class="mt-2 text-sm text-neutral-600 space-y-1 list-disc list-inside">
                                ...
                                <li>Email notifications will be sent to the updated email address</li>
                                @else
        ...
        <div class="mt-8 flex items-center justify-end gap-3">
            ...
            <button type="submit" class="btn-filled">
                <span class="material-symbols-rounded">{{ isset($user) ? 'save' : 'add' }}</span>
                {{ isset($user) ? 'Update Player' : 'Create Player' }}
            </button>
        </div>
    </form>
</div>
